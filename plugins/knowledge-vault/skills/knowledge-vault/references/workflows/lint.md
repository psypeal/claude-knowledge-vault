...
```bash
bash "${KV_PLUGIN_ROOT}/scripts/lint-checks.sh"
```
...
Combine script output and model checks into `wiki/outputs/lint-YYYY-MM-DD.md`. Run:

```bash
bash "${KV_PLUGIN_ROOT}/scripts/rebuild-index.sh"
bash "${KV_PLUGIN_ROOT}/scripts/update-state.sh" .vault last_lint="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
```
