...
**File the answer** ONLY when the user says "file it" or "save this". Then write to `wiki/outputs/` and run `bash "${KV_PLUGIN_ROOT}/scripts/rebuild-index.sh"`.

**Update agent.md** ONLY after 3+ queries in the same session. Then read `${KV_PLUGIN_ROOT}/skills/knowledge-vault/references/agent-update-rules.md`.
