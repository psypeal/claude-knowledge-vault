...
Read those 2-4 files in `.vault/wiki/concepts/` and `.vault/wiki/summaries/`. **Most queries should end here.** If the answer is plain enough from compiled knowledge, write it concisely with `[[wikilinks]]` to sources and stop.
...
Pull just the relevant pages from the original document with the bounded extraction helper:

```text
<python> "${KV_PLUGIN_ROOT}/scripts/extract_pdf.py" .vault/originals/<slug>.pdf --first <start_index> --last <end_index>
```

If the helper reports that `pdftotext` is missing, tell the user to install Poppler and answer from tier 2/3 evidence instead of failing. For non-PDF originals, read `.vault/raw/<slug>.md` directly; there is no tree-driven page extraction.
...
**File the answer** ONLY when the user says "file it" or "save this". Then write to `.vault/wiki/outputs/` and run `<python> "${KV_PLUGIN_ROOT}/scripts/kv.py" rebuild`.

**Update `.vault/agent.md`** ONLY after 3+ queries in the same session. Then read `${KV_PLUGIN_ROOT}/skills/knowledge-vault/references/agent-update-rules.md`.

**Agent pre-routing** ONLY if `total_queries >= 5` in `.vault/agent.md`. Read it before the index.
...
- **Graceful degradation**: vaults from v2.3.0 have no trees and no `.vault/originals/`. Tier 3 silently skips; tier 4 falls back to reading `.vault/raw/<slug>.md`. The query still works.
